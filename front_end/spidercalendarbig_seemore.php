...
<div style="background-color:<?php echo $show_event_bgcolor ?>; height:<?php echo $popup_height-30 ?>px;padding:15px;">
 ...
 if($row->id==$ev_id[$i])
 echo '<div ><a style="font-size:'.$title_size.'px;color:'.$title_color.'; line-height:30px" href="'.plugins_url("spidercalendarbig.php",__FILE__).'?theme_id='.$theme_id.'&calendar_id='.$calendar_id.'&ev_ids='.$ev_ids.'&eventID='.$ev_id[$i].'&date='.$date.'&day='.$day.(($widget)?('&widget=1'):'').'&cur_page_url='.$path_sp_cal.'">'.($i+1).' '.$row->title .'suruyo </a></div>';
